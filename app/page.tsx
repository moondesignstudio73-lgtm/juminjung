...
import type { AuraDefinition, FacilityId, FoodRationPolicy, GameState, Guest, GuestExpression, HotelActionId, InvestigationCaseId, InvestigationConclusionId, InvestigationPointId, PowerCircuitId, Room, ScavengeMissionId, StaffDutyId } from '@/game/types';
...
function HotelManagement({ state, guest, stayingGuests, hasStayingGuest, onSelectGuest, onBuild, onAction, onPower, onRation, onStaff, onScavenge, onInvestigate, onConclude, onMove, onCheckout, onContinue }: { state:UiSave; guest:Guest; stayingGuests:Guest[]; hasStayingGuest:boolean; onSelectGuest:(guestId:string)=>void; onBuild:(id:FacilityId)=>void; onAction:(id:HotelActionId)=>void; onPower:(id:PowerCircuitId,enabled:boolean)=>void; onRation:(policy:FoodRationPolicy)=>void; onStaff:(dutyId:StaffDutyId,guestId:string|null)=>void; onScavenge:(missionId:ScavengeMissionId)=>void; onInvestigate:(caseId:InvestigationCaseId,pointId:InvestigationPointId)=>void; onConclude:(caseId:InvestigationCaseId,conclusionId:InvestigationConclusionId)=>void; onMove:()=>void; onCheckout:()=>void; onContinue:()=>void }) {
