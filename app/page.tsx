...
type Phase = 'title' | 'prologue' | 'desk' | 'night' | 'report';
type Decision = 'checkin' | 'refuse' | null;
type Save = {
  phase: Phase; prologue: number; asked: string[]; inspected: string[];
  negotiated: boolean; held: boolean; decision: Decision;
};

const initial: Save = {
  phase: 'title', prologue: 0, asked: [], inspected: [], negotiated: false,
  held: false, decision: null,
};
...
export default function Home() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem('juju-hotel-save-v1');
    if (raw) {
      try { setSave({ ...initial, ...JSON.parse(raw) }); } catch { localStorage.removeItem('juju-hotel-save-v1'); }
    }
    setHydrated(true);
  }, []);

  useEffect(() => {
    if (hydrated && save.phase !== 'title') localStorage.setItem('juju-hotel-save-v1', JSON.stringify(save));
  }, [save, hydrated]);

  const update = (patch: Partial<Save>) => setSave((current) => ({ ...current, ...patch }));
  const reset = () => { localStorage.removeItem('juju-hotel-save-v1'); setSave(initial); setDialogue(defaultDialogue); setSelectedItem(null); };
  ...
  const decide = (decision: Exclude<Decision, null>) => update({ decision, phase: 'night' });
  ...
  if (save.phase === 'night') return <NightEvent decision={save.decision!} onContinue={() => update({ phase: 'report' })} />;
  if (save.phase === 'report') return <MorningReport decision={save.decision!} negotiated={save.negotiated} inspected={save.inspected} onReset={reset} />;
    ...
    <main className="game-shell">
      ...
      <section className="desk-scene" aria-label="밤의 JUJU HOTEL 프런트">
        ...
        <aside className="hotel-status right-panel">
          <span className="panel-label">야간 장부 · 자원 점수</span><strong>12</strong><small>빈 객실 · 총 12실</small>
        ...
        <div className="decision-bar">
          ...
          <Button className="refuse" onClick={() => decide('refuse')}>거절</Button>
          <Button className="checkin" onClick={() => decide('checkin')}><BedDouble/> 체크인 · 203호</Button>
        </div>
      </section>
...
function NightEvent({ decision, onContinue }: { decision: Exclude<Decision,null>; onContinue:()=>void }) {
  ...
  return <main className="event-screen"><div className="event-light"/><Radio className="event-icon"/><p className="scene-index">DAY 1 · 오전 2:13</p><section><span>야간 사건 · {accepted?'203호':'동쪽 철문'}</span><h1>{accepted?'문틈 아래의 불빛':'세 번의 노크, 그리고 침묵'}</h1><p>{accepted?'무언가 깨지는 소리에 2층 전체가 깨어난다. 엘리너는 이미 복도에 나와 월터의 피 흘리는 옆구리를 두 손으로 누르고 있다. 그녀는 허락도 구하지 않고 약장을 연다. 동이 틀 무렵, 월터는 다시 안정적으로 숨을 쉰다.':'라디오에서 잡음과 함께 여자의 목소리가 흘러나와, 당신이 내주려 했던 객실 번호를 반복한다. 해 뜰 무렵 철문에는 깨끗한 의료용 붕대가 묶여 있다. 진흙 위에는 발자국 하나 없다.'}</p><blockquote>{accepted?'“의사라는 증거를 원했죠.” — 엘리너':'같은 목소리가 한 번 더 말한다. “42마일 지점.”'}</blockquote><Button onClick={onContinue}>밤을 넘긴다 <ChevronRight/></Button></section></main>;
}

function MorningReport({ decision, negotiated, inspected, onReset }: { decision:Exclude<Decision,null>; negotiated:boolean; inspected:string[]; onReset:()=>void }) {
  const accepted=decision==='checkin';
  return <main className="report-screen"><header><p className="eyebrow">JUJU HOTEL · 아침 장부</p><h1>DAY 2</h1><span>오전 6:32 · 비가 잦아드는 중</span></header><section className="report-paper"><div className="stamp">{accepted?'입실 허가':'입실 거절'}</div><p className="panel-label">간밤의 보고</p><h2>{accepted?'203호에 투숙객이 있습니다.':'열두 객실이 모두 비어 있습니다.'}</h2><p>{accepted?'엘리너 리드는 호텔 사람들의 경계 어린 관심을 얻었습니다. 그녀가 벽 안에 있었기에 월터는 살아남았습니다.':'오전 2시 13분, 동쪽 철문의 카메라가 꺼졌습니다. 누군가, 혹은 무언가가 당신이 내주려 한 객실을 알고 있었습니다.'}</p><div className="ledger-grid"><Result icon={BedDouble} label="객실" before="12" after={accepted?'11':'12'}/><Result icon={Fuel} label="연료" before="62" after={accepted?'70':'62'} good={accepted}/><Result icon={HeartPulse} label="의약품" before="18" after={accepted?(negotiated?'22':'21'):'18'} good={accepted}/><Result icon={Shield} label="보안" before="35" after={accepted?'39':'31'} good={accepted}/></div><div className="consequence"><span>새 이야기 해금</span><strong>{accepted?'203호의 의사':'42마일 지점의 목소리'}</strong><p>{inspected.includes('photo')?'사진 뒷면에서 본 “42마일 지점”을 기억해 냈습니다. 이 단서는 나중에 중요해집니다.':'카운터 위에서 단서 하나를 놓쳤습니다. 거절한 사람과 함께 답도 떠나갈 수 있습니다.'}</p></div><footer><div><span>버티컬 슬라이스 완료</span><p>당신의 선택이 투숙객 명단과 자원, 다음 이야기 갈래를 바꿨습니다.</p></div><Button onClick={onReset}><RotateCcw/> 다른 선택 해보기</Button></footer></section></main>;
}
