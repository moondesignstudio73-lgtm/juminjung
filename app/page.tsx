...
import type { AuraDefinition, FacilityId, FoodRationPolicy, GameState, Guest, GuestExpression, HotelActionId, PowerCircuitId, Room, ScavengeMissionId, StaffDutyId } from '@/game/types';
...
function HotelManagement({ state, guest, stayingGuests, hasStayingGuest, onSelectGuest, onBuild, onAction, onPower, onRation, onStaff, onScavenge, onMove, onCheckout, onContinue }: { state:UiSave; guest:Guest; stayingGuests:Guest[]; hasStayingGuest:boolean; onSelectGuest:(guestId:string)=>void; onBuild:(id:FacilityId)=>void; onAction:(id:HotelActionId)=>void; onPower:(id:PowerCircuitId,enabled:boolean)=>void; onRation:(policy:FoodRationPolicy)=>void; onStaff:(dutyId:StaffDutyId,guestId:string|null)=>void; onScavenge:(missionId:ScavengeMissionId)=>void; onMove:()=>void; onCheckout:()=>void; onContinue:()=>void }) {
