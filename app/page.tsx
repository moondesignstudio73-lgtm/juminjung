...
function HotelGrid({ rooms, auraDefinition, auraMode='ambient', selected, affected, onSelect }: { rooms: Room[]; auraDefinition?:AuraDefinition|null; auraMode?:'preview'|'ambient'; selected?: number | null; affected?: number[]; onSelect?: (roomNumber:number)=>void }) {
  ...
  return <div className="hotel-cutaway" aria-label="JUJU HOTEL 30개 객실 배치도">{[3,2,1].map((floor) => <div className="hotel-floor" key={floor}><strong>{floor}F</strong><div className="room-row">{rooms.filter((room) => room.floor === floor).map((room) => {const affectedByAura=aura.has(room.roomNumber)&&Boolean(auraDefinition);return <button key={room.roomNumber} disabled={Boolean(onSelect) && !isRoomSelectable(room)} onClick={() => onSelect?.(room.roomNumber)} className={['room-cell', room.status.toLowerCase(), selected === room.roomNumber ? 'selected' : '', affectedByAura ? `aura aura-${auraDefinition!.category.toLowerCase()} aura-${auraMode}` : ''].join(' ')}><b>{room.roomNumber}</b><span>{getRoomOccupantLabel(room, ROOM_GUEST_CATALOG)}</span>{affectedByAura && <i title={auraDefinition!.name}><AuraGlyph aura={auraDefinition!}/><em>{auraDefinition!.shortLabel}</em></i>}</button>})}</div></div>)}</div>;
}
...
function RoomAssignment({ day, rooms, guest, selected, mode, onSelect, onConfirm, onCancel }: { day:number; rooms:Room[]; guest:Guest; selected:number|null; mode:'checkin'|'move'; onSelect:(roomNumber:number)=>void; onConfirm:()=>void; onCancel:()=>void }) {
  ...
  return <main className="room-screen">
    ...
    <section className="room-layout"><div className="room-board"><HotelGrid rooms={rooms} auraDefinition={guest.aura} auraMode="preview" selected={selected} affected={affected} onSelect={onSelect}/><div className="room-legend"><span>빈 객실</span><span>사용 중</span><span>{guest.aura?.name??'Aura 없음'}</span></div></div>
