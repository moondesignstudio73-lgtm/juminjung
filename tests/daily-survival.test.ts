...
test("저장 복원은 v12 전력 배분·배급 정책·남은 AP를 유지한다", () => {
  let state = createInitialGameState();
...
test("v9의 하루 전체 AP 저장은 v12의 3 AP로 마이그레이션된다", () => {
  ...
  assert.deepEqual({ version: restored.version, actionPoints: restored.actionPoints, maxActionPoints: restored.maxActionPoints }, { version: 12, actionPoints: 3, maxActionPoints: 3 });
  assert.equal(restored.foodRationPolicy, "NORMAL");
});

test("v9에서 일부 사용한 AP는 v12 마이그레이션 후에도 그대로 유지된다", () => {
  const legacy = JSON.parse(serializeGameState(createInitialGameState()));
  legacy.version = 9;
