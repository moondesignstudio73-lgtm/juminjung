...
import {
  ...
  runScavengeMission,
  SAFE_ROUTE_SCAVENGE_CHANCE_BONUS,
  ...
  state.guests = state.guests.map((guest) =>
    ids.includes(guest.id)
      ? {
          ...
          currentRoomNumber: 101 + ids.indexOf(guest.id),
          remainingNights: 5,
          checkedInDay: 1,
        }
      : guest,
  );
  state.rooms = ids.reduce(
    (rooms, id, index) => assignGuest(rooms, 101 + index, id),
    state.rooms,
  );
...
test('손상 저장의 중복·퇴실 근무 배치는 역할 순서에 따라 하나만 복구한다', () => {
  ...
  const assignments = pruneStaffAssignments(
    { MAINTENANCE: 'walter', SECURITY: 'walter', MEDICAL: 'eleanor' },
    state.guests,
  );
...
test('탐색 성공률은 담당자의 탐색·작업·전투 능력과 임무 난이도를 함께 사용한다', () => {
  ...
  assert.ok(
    getScavengeChance(hazel, 'NEARBY_BLOCK') >
      getScavengeChance(mia, 'NEARBY_BLOCK'),
  );
  assert.ok(
    getScavengeChance(hazel, 'NEARBY_BLOCK') >
      getScavengeChance(hazel, 'FUEL_DEPOT'),
  );
});
...
test('안전 통로는 탐색 성공률을 높이고 임무별 실제 노출까지만 낮춘다', () => {
  ...
  const hazel = state.guests.find((guest) => guest.id === 'hazel')!;
  assert.equal(
    getScavengeChance(walter, 'FUEL_DEPOT', true) -
      getScavengeChance(walter, 'FUEL_DEPOT'),
    SAFE_ROUTE_SCAVENGE_CHANCE_BONUS,
  );
  assert.deepEqual(getScavengeChanceBreakdown(mia, 'FUEL_DEPOT', true), {
    active: true,
    chanceBonus: 10,
    exposureReduction: 2,
    baseChance: 15,
    appliedChanceBonus: 10,
    chance: 25,
  });
  assert.deepEqual(getScavengeChanceBreakdown(hazel, 'NEARBY_BLOCK', true), {
    active: true,
    chanceBonus: 10,
    exposureReduction: 1,
    baseChance: 95,
    appliedChanceBonus: 0,
    chance: 95,
  });
  assert.deepEqual(getScavengeRouteModifiers('FUEL_DEPOT', true), {
    active: true,
    chanceBonus: SAFE_ROUTE_SCAVENGE_CHANCE_BONUS,
    exposureReduction: SAFE_ROUTE_SCAVENGE_EXPOSURE_REDUCTION,
  });
  assert.deepEqual(getScavengeRouteModifiers('NEARBY_BLOCK', true), {
    active: true,
    chanceBonus: SAFE_ROUTE_SCAVENGE_CHANCE_BONUS,
    exposureReduction: 1,
  });
  assert.deepEqual(getScavengeRouteModifiers('FUEL_DEPOT', false), {
    active: false,
    chanceBonus: 0,
    exposureReduction: 0,
  });
...
test('탐색 결과는 같은 세이브 시드·DAY·임무·담당자에서 결정적이며 하루 한 번만 실행된다', () => {
  ...
  assert.doesNotMatch(
    ...
    /\b(food|water|medicine|fuel|parts|security|Health|Threat)\b/,
  );
...
test('위험한 탐색의 철수 결과는 정찰 담당자 부상과 Monster Threat 상승을 기록한다', () => {
  ...
  for (let seed = 1; seed <= 200 && !found; seed += 1) {
    let state = activate(['mia']);
    state.visitorSeed = seed;
    const beforeHealth = state.guests.find(
      (guest) => guest.id === 'mia',
    )!.health;
    state = assignStaffDuty(state, 'SCAVENGE', 'mia').state;
    const result = runScavengeMission(state, 'FUEL_DEPOT');
    if (result.report?.outcome === 'SETBACK') {
      assert.equal(
        result.state.guests.find((guest) => guest.id === 'mia')!.health,
        beforeHealth - 12,
      );
      found = result;
    }
...
test('야간 근무 계획은 정비·경비·의료·주방 능력을 서로 다른 효과로 계산한다', () => {
  ...
  state.guests = state.guests.map((guest) =>
    guest.id === 'mia'
      ? { ...guest, health: 60, infectionState: 'INJURED' as const }
      : guest,
  );
  const plan = getNightStaffPlan(state);
  assert.deepEqual(
    {
      food: plan.foodSaving,
      condition: plan.conditionDelta,
      security: plan.securityDelta,
      threat: plan.threatDelta,
      patient: plan.healingGuestId,
...
test('의료 담당자는 다른 환자가 없을 때 자신의 부상도 치료할 수 있다', () => {
  ...
  state.guests = state.guests.map((guest) =>
    guest.id === 'eleanor'
      ? { ...guest, health: 55, infectionState: 'INJURED' as const }
      : guest,
  );
  const plan = getNightStaffPlan(state);
  assert.deepEqual(
    { patient: plan.healingGuestId, healing: plan.healing },
    { patient: 'eleanor', healing: 6 },
  );
});
...
test('상태 상한·위협 하한에서는 적용되지 않은 근무 효과를 아침 장부에 기록하지 않는다', () => {
  ...
  state.hotelStats.security = 100;
  state.flags.monster_threat = 0;
  ...
  assert.equal(
    resolved.lastDaySummary?.staffDutyResults?.some(
      (result) => result.dutyId === 'SECURITY',
    ),
    false,
  );
  assert.equal(
    resolved.eventHistory.some((entry) =>
      entry.message.includes('근무 정산 · 월터'),
    ),
    false,
  );
});

test('Save v15는 근무 배치와 마지막 탐색 보고서를 보존하고 v11에는 안전한 기본값을 준다', () => {
  let state = activate(['hazel']);
  state = assignStaffDuty(state, 'SCAVENGE', 'hazel').state;
  ...
  legacy.version = 11;
  delete legacy.staffAssignments;
  delete legacy.lastScavengeDay;
  delete legacy.lastScavengeReport;
  const migrated = restoreGameState(JSON.stringify(legacy));
  assert.deepEqual(
    {
      version: migrated.version,
      assignments: migrated.staffAssignments,
      day: migrated.lastScavengeDay,
      report: migrated.lastScavengeReport,
    },
    { version: 15, assignments: {}, day: 0, report: null },
  );
});
