...
import {
  EVIDENCE_CATALOG,
  INVESTIGATION_CASES,
} from '../game/investigation-data.ts';
import {
...
test('207호 사건 데이터는 네 조사 지점과 고유 증거를 하나의 사건 정의에서 연결한다', () => {
  const definition = INVESTIGATION_CASES.find(
    (entry) => entry.id === 'ROOM_207',
  )!;
  assert.equal(definition.points.length, 4);
  assert.equal(
    new Set(definition.points.map((point) => point.evidenceId)).size,
    4,
  );
  assert.deepEqual(
    new Set(definition.points.map((point) => point.evidenceId)),
    new Set(EVIDENCE_CATALOG.map((evidence) => evidence.id)),
  );
...
test('현장 조사에는 1 AP가 들고 증거·스토리 플래그·호텔 기록이 함께 남는다', () => {
  ...
  assert.deepEqual(
    getInvestigationCaseState(result.state, 'ROOM_207')?.collectedEvidenceIds,
    ['ROOM_207_INTERIOR_KEY'],
  );
  assert.equal(result.state.flags.evidence_room_207_interior_key, true);
  assert.match(
    result.state.eventHistory.at(-1)!.message,
    /문 안쪽의 황동 열쇠/,
  );
});
...
test('같은 지점은 중복 조사할 수 없고 AP가 없으면 새 지점도 조사할 수 없다', () => {
  ...
  state = investigateCasePoint(state, 'ROOM_207', 'ROOM_207_DOOR').state;
  assert.equal(
    canInvestigateCasePoint(state, 'ROOM_207', 'ROOM_207_DOOR'),
    false,
  );
  assert.equal(
    investigateCasePoint(state, 'ROOM_207', 'ROOM_207_DOOR').ok,
    false,
  );
  state = { ...state, actionPoints: 0 };
  assert.equal(
    canInvestigateCasePoint(state, 'ROOM_207', 'ROOM_207_WINDOW'),
    false,
  );
  assert.equal(
    investigateCasePoint(state, 'ROOM_207', 'ROOM_207_WINDOW').ok,
    false,
  );
});
...
test('증거 세 개 전에는 결론을 내릴 수 없고 핵심 세 증거는 괴물 침입 결론을 지지한다', () => {
  ...
  assert.equal(
    concludeInvestigationCase(state, 'ROOM_207', 'MONSTER_ENTRY').ok,
    false,
  );
  ...
  assert.equal(
    assessInvestigationConclusion(caseState, 'ROOM_207', 'MONSTER_ENTRY'),
    'SUPPORTED',
  );
  assert.equal(
    assessInvestigationConclusion(caseState, 'ROOM_207', 'HUMAN_ATTACK'),
    'CONTRADICTED',
  );
});
...
test('괴물 침입 결론은 실제 단서·평판·보안·위협과 사건 상태를 함께 갱신한다', () => {
  ...
  assert.equal(
    result.state.reputations.community,
    state.reputations.community + 3,
  );
  assert.equal(result.state.flags.monster_threat, 0);
  assert.deepEqual(
    {
      status: getInvestigationCaseState(result.state, 'ROOM_207')?.status,
      conclusion: getInvestigationCaseState(result.state, 'ROOM_207')
        ?.conclusionId,
    },
    { status: 'SOLVED', conclusion: 'MONSTER_ENTRY' },
  );
});
...
test('Save v15는 조사 진척과 증거를 보존하고 결론 뒤 재조사를 막는다', () => {
  ...
  assert.equal(
    canInvestigateCasePoint(restored, 'ROOM_207', 'ROOM_207_LUGGAGE'),
    false,
  );
});
...
test('v12의 기존 207호 조사 플래그는 증거를 날조하지 않고 열린 사건 파일로 이관된다', () => {
  ...
  assert.equal(restored.version, 15);
  assert.deepEqual(restored.investigationCases, [
    {
      caseId: 'ROOM_207',
      status: 'OPEN',
      openedDay: 14,
...
test('손상 저장은 조사하지 않은 증거·조기 결론을 제거하고 유효 증거 플래그를 복원한다', () => {
  ...
  damaged.investigationCases[0].collectedEvidenceIds = [
  ...
  assert.deepEqual(restored.investigationCases, [
    {
      ...
      openedDay: 12,
      inspectedPointIds: ['ROOM_207_DOOR'],
      collectedEvidenceIds: ['ROOM_207_INTERIOR_KEY'],
      conclusionId: null,
      resolvedDay: null,
    },
  ]);
...
test('열린 사건은 오늘의 추천 목표로 나타나고 결론 뒤에는 사라진다', () => {
  let state = openRoom207Case();
  assert.ok(
    getDailyObjectives(state).some(
      (objective) =>
        objective.id === 'investigation_room_207' &&
        objective.description.includes('0/4'),
    ),
  );
  ...
  assert.equal(
    getDailyObjectives(state).some(
      (objective) => objective.id === 'investigation_room_207',
    ),
    false,
  );
});
