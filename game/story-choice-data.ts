...
export const STORY_CHOICE_EVENTS: StoryChoiceEvent[] = [
  ...
  { id: "eleanor-standard", guestId: "eleanor", stage: "RESOLUTION", title: "호텔의 진료 원칙", description: "Eleanor는 임시 진료실을 계속 운영하려면 호텔이 누구를, 어디까지 치료할지 정해야 한다고 말합니다.", quote: "‘원칙이 없으면 약보다 먼저 사람이 바닥나요.’", choices: [
    { id: "clinic", label: "상설 진료 구역을 만든다", description: "의약품 3을 사용하고 Trust +10, community 평판 +8과 humanitarian 평판 +8을 얻습니다. Eleanor가 떠난 뒤에도 매일 모든 투숙객의 일반 질병(NORMAL_DISEASE) 발생 확률에 누적되지 않는 -5%p 보정을 적용합니다. 대신 순회 진료의 단일 부상자 Health +5 회복과 refugee 평판 +5를 포기합니다.", requiredResources: { medicine: 3 }, effect: { resources: { medicine: -3 }, trust: 10, reputations: { community: 8, humanitarian: 8 }, flags: { eleanor_clinic_established: true, medical_network_active: true, eleanor_mobile_medic: false } } },
    { id: "mobile", label: "순회 진료만 허용한다", description: "의약품 비축을 보존하고 Trust +5와 refugee 평판 +5를 얻습니다. Eleanor가 투숙 중인 동안 매일 Eleanor 자신을 제외한 주민 중 Health가 가장 낮은 한 명을 +5 회복하지만, 상설 진료소의 호텔 전체 일반 질병(NORMAL_DISEASE) -5%p 예방을 영구적으로 포기합니다.", effect: { trust: 5, reputations: { refugee: 5 }, flags: { eleanor_mobile_medic: true, eleanor_clinic_established: false, medical_network_active: false } } },
  ] },
  ...
  { id: "owen-future", guestId: "owen", stage: "RESOLUTION", title: "군의 포위망", description: "마커스 헤이스 대령의 병력이 호텔로 접근합니다. 오웬 밀러는 싸울지, 떠날지 결정할 준비가 되었습니다.", quote: "‘이번에는 명령이 아니라 내가 지킬 곳을 고르겠습니다.’", choices: [
    { id: "resistance", label: "호텔 방어대를 조직한다", description: "Trust +10, Security +8, refugee 평판 +8을 얻고 military 평판 -10을 감수합니다. 오웬 밀러가 방위대를 훈련하고 DAY 25 공성 계획을 남겨 로비 사수 비용을 보안 물자 6→4, 부품 3→2로 줄이며 선두 방어자 1명의 부상을 Health -20→-10으로 완화합니다. 오웬 밀러가 투숙 중인 동안 객실 Aura와 근무 배치도 유지됩니다. 이 결말 선택은 되돌릴 수 없으며, 대신 즉시 Monster Threat -5와 humanitarian 평판 +4를 얻는 탈출 경로를 포기합니다.", effect: { trust: 10, hotelStats: { security: 8 }, reputations: { military: -10, refugee: 8 }, flags: { military_resistance_succeeded: true, hotel_defense_force: true, owen_siege_plan: true, owen_escaped: false } } },
    { id: "escape", label: "오웬 밀러의 탈출을 돕는다", description: "군 추격대가 호텔에서 물러나 Monster Threat -5를 얻고 humanitarian 평판 +4를 얻으며 military 평판 -10 손실을 피합니다. 오웬 밀러는 안전 통로로 영구 출발해 객실 Aura와 근무 배치가 즉시 해제되고 재방문하지 않습니다. 대신 Trust +10, Security +8, refugee 평판 +8과 DAY 25 공성의 보안 물자 6→4, 부품 3→2, 선두 방어자 Health -20→-10 완화를 포기합니다. 다른 주민이 만든 방위 효과는 유지됩니다.", effect: { threat: -5, reputations: { humanitarian: 4 }, flags: { military_resistance_succeeded: false, owen_siege_plan: false, owen_escaped: true }, departure: { endingState: "ESCAPED", finalState: "STORY_ESCAPED" } } },
  ] },
  ...
  { id: "lily-truth", guestId: "lily", stage: "RESOLUTION", title: "누가 진실을 소유하는가", description: "복원된 기록을 세상에 공개할지, 호텔 안에서 다음 단서를 기다릴지 결정해야 합니다.", quote: "‘진실은 사람을 구할 수도 있고, 마지막 피난처를 불태울 수도 있어요.’", choices: [
    { id: "broadcast", label: "라디오로 해독 결과를 공개한다", description: "직접 복원한 문서를 모두가 검증할 수 있게 공개합니다. 호텔 위치도 드러나지만 DAY 16부터 바깥 생존자들의 보강 증언을 수신할 수 있습니다.", requiredFlags: { lily_documents_decoded: true }, effect: { trust: 12, threat: 12, reputations: { community: 6, humanitarian: 5 }, flags: { lily_documents_decoded: true, lily_truth_broadcast: true, lily_truth_archived: false }, discoverTrait: "OriginDocuments" } },
    { id: "archive", label: "암호화해 호텔 금고에 보관한다", description: "암호화한 기록을 보존해 호텔 위치를 숨기지만, 외부 생존자들의 보강 증언을 받을 기회를 포기합니다.", effect: { trust: 5, hotelStats: { security: 4 }, flags: { lily_documents_decoded: true, lily_truth_archived: true, lily_truth_broadcast: false }, discoverTrait: "OriginDocuments" } },
  ] },
  { id: "vale-research", guestId: "vale", stage: "RESOLUTION", title: "괴물의 이름", description: "Vale의 연구는 괴물이 감염체가 아니라 인간의 기억을 모방하는 별개의 생명이라는 결론에 도달합니다.", quote: "‘이 결과를 완성하면, 우리도 이전의 인간으로 돌아갈 수 없습니다.’", choices: [
    { id: "complete", label: "연구를 완성하고 Lily와 공유한다", description: "에이드리언 베일 박사의 Trust 12를 얻고 THE TRUTH 경로를 열어 매일 밤 Monster Threat를 2 낮추지만, 소각으로 얻을 즉시 위협 최대 12 감소와 humanitarian 평판 4를 포기합니다.", requiredFlags: { vale_sample_stabilized: true }, effect: { trust: 12, fatherStoryProgress: 10, flags: { vale_research_complete: true, lily_vale_research_shared: true, vale_research_destroyed: false }, relationship: { targetId: "lily", delta: 25 }, discoverTrait: "PreOutbreakResearch" } },
    { id: "destroy", label: "연구와 샘플을 소각한다", description: "현재 Monster Threat를 0 아래로 내리지 않는 범위에서 최대 12 낮추고, 호텔 주민의 노출 위험을 없애 humanitarian 평판 4를 얻습니다. 대신 에이드리언 베일 박사의 Trust 10, THE TRUTH 경로, Lily 공동 연구와 매일 Monster Threat 2 감소를 영구히 포기합니다.", effect: { trust: -10, threat: -12, reputations: { humanitarian: 4 }, flags: { vale_research_destroyed: true, vale_research_complete: false, lily_vale_research_shared: false } } },
  ] },
