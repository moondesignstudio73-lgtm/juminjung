...
export const STORY_CHOICE_EVENTS: StoryChoiceEvent[] = [
  ...
  { id: "jack-market", guestId: "jack", stage: "RESOLUTION", title: "폐허의 시장", description: "Jack은 호텔을 중립 시장으로 만들면 모든 물자가 이곳을 거치게 할 수 있다고 제안합니다.", quote: "‘왕관은 필요 없어. 문을 여닫는 사람이 가격을 정하니까.’", choices: [
    { id: "fair_market", label: "공정 거래소를 연다", description: "가격과 배급량을 공개해 공동체가 감시하는 시장을 만듭니다.", effect: { trust: 10, resources: { food: 2, medicine: 1 }, reputations: { merchant: 8, community: 8 }, flags: { jack_fair_market: true, trade_network_active: true }, relationship: { targetId: "victor", delta: -10 } } },
    { id: "monopoly", label: "독점 거래권을 허가한다", description: "Jack과 Victor의 물자망으로 호텔의 지배력을 키웁니다.", effect: { trust: 5, resources: { food: 4, fuel: 2 }, reputations: { merchant: 12, community: -8 }, flags: { jack_monopoly: true, ruin_market_controlled: true }, relationship: { targetId: "victor", delta: 25 } } },
  ] },
  ...
  { id: "hayes-command", guestId: "hayes", stage: "RESOLUTION", title: "누가 호텔을 지키는가", description: "Hayes는 최종 명령권을 요구합니다. 서명하면 JUJU HOTEL은 군의 거점이 됩니다.", quote: "‘보호받고 싶다면 누군가는 명령해야 한다.’", choices: [
    { id: "civilian_rule", label: "민간 협의체에 복종시킨다", description: "Hayes의 병력도 호텔 규칙 아래 두고 자치 방위대를 완성합니다.", effect: { trust: -8, hotelStats: { security: 10 }, reputations: { military: -8, community: 8 }, flags: { military_resistance_succeeded: true, civilian_command: true }, relationship: { targetId: "owen", delta: 20 } } },
    { id: "sign_command", label: "군 지휘권에 서명한다", description: "강력한 방어를 얻지만 호텔의 최종 결정권을 넘깁니다.", effect: { trust: 12, resources: { security: 3 }, hotelStats: { security: 15 }, reputations: { military: 18, community: -12 }, flags: { military_influence_high: true, military_rule_signed: true }, relationship: { targetId: "owen", delta: -30 } } },
  ] },
  ...
  { id: "victor-crown", guestId: "victor", stage: "RESOLUTION", title: "폐허의 왕좌", description: "Victor는 Jack의 시장과 벙커망을 합쳐 호텔이 지역 물자를 지배할 수 있다고 주장합니다.", quote: "‘도덕은 풍족할 때의 화폐죠. 지금 필요한 건 소유권입니다.’", choices: [
    { id: "public_trust", label: "벙커를 공동 신탁으로 전환한다", description: "좌석과 물자를 공개 장부로 관리해 Victor의 독점을 끝냅니다.", effect: { trust: -8, reputations: { community: 12, humanitarian: 6 }, flags: { victor_public_trust: true, bunker_network_open: true }, relationship: { targetId: "rosa", delta: 25 } } },
    { id: "rule_market", label: "Victor와 독점 연합을 맺는다", description: "호텔이 벙커와 시장의 가격을 통제하는 지배 세력이 됩니다.", effect: { trust: 12, resources: { food: 3, medicine: 2, fuel: 2 }, reputations: { merchant: 15, community: -15 }, flags: { victor_monopoly_alliance: true, ruin_market_controlled: true }, relationship: { targetId: "rosa", delta: -30 } } },
  ] },
