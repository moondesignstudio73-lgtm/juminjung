...
export function restoreGameState(raw: string | null): GameState {
  ...
  try {
    ...
    if (![2, 3, 4, 5, 6, 7].includes(decoded.version ?? 0) || !Array.isArray(decoded.rooms) || !Array.isArray(decoded.guests)) return createInitialGameState();
