...
export function resolveDay(state: GameState): GameState {
  ...
  const entries: HotelLogEntry[] = [
    ...
    ...(auraNight.familyZoneGuestIds.length ? [{day:state.day,type:"EVENT" as const,message:`가족 안전 구역 보호 · ${auraNight.familyZoneGuestIds.map((id)=>guests.find((guest)=>guest.id===id)?.name??id).join(" · ")} · Stress -5`}] : []),
