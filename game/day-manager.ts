...
export function resolveDay(state: GameState): GameState {
  ...
  const rationPlan = getRationPlan(baseFoodDemand, state.foodRationPolicy);
  ...
  const threatAfterAuraNight = Math.max(0,threatBeforeAuraNight+auraNight.threatDelta+powerPlan.threatDelta);
  const threatWithoutResearchPrediction = Math.max(0,threatBeforeAuraNight+auraNight.threatDelta+auraNight.researchPredictionThreatReduction+powerPlan.threatDelta);
  const threatWithoutPathfinder = Math.max(0,threatBeforeAuraNight+auraNight.threatDelta+auraNight.researchPredictionThreatReduction+auraNight.pathfinderThreatReduction+powerPlan.threatDelta);
  const threatWithoutAlarm = Math.max(0,threatBeforeAuraNight+auraNight.threatDelta+auraNight.researchPredictionThreatReduction+auraNight.pathfinderThreatReduction+auraNight.perimeterAlarmThreatReduction+powerPlan.threatDelta);
  ...
  const securityWithoutCivilGuard = Math.max(0,Math.min(100,state.hotelStats.security+auraNight.securityDelta-auraNight.civilGuardSecurityGain+powerPlan.securityDelta));
  const securityAfterAuraNight = Math.max(0,Math.min(100,state.hotelStats.security+auraNight.securityDelta+powerPlan.securityDelta));
