...
export type EvidenceId =
  | 'ROOM_207_INTERIOR_KEY'
  | 'ROOM_207_WINDOW_TRACE'
  | 'ROOM_207_BLACK_RESIDUE'
  | 'ROOM_207_GUEST_LEDGER';
export type InvestigationPointId =
  | 'ROOM_207_DOOR'
  | 'ROOM_207_WINDOW'
  | 'ROOM_207_FLOOR'
  | 'ROOM_207_LUGGAGE';
export type InvestigationConclusionId =
  | 'VOLUNTARY_EXIT'
  | 'HUMAN_ATTACK'
  | 'MONSTER_ENTRY'
  | 'UNRESOLVED';
...
export type VisitorStatementAssessment =
  | 'RECORDED'
  | 'CORROBORATED'
  | 'CONTRADICTED';
export type VisitorStatementId =
  | 'RUTH_SCRATCH_CLAIM'
  | 'HAZEL_TRACKS_TESTIMONY'
  | 'WHITE_FALSE_VOICE_WARNING';
export type MonsterCodexEntryId = 'MIMIC_STALKER' | 'SIGNAL_PARASITE';
export type MonsterInsightId =
  | 'TRIPLE_CLAW_PATTERN'
  | 'SHIFTING_GAIT'
  | 'INTERIOR_EXIT_ROUTE'
  | 'BORROWED_VOICE'
  | 'SYNCHRONIZED_CALLS'
  | 'RELAY_ECHO';
export type MonsterKnowledgeSourceId =
  | 'RUTH_SCRATCH_CONTRADICTION'
  | 'HAZEL_TRACKS_TESTIMONY'
  | 'ROOM_207_MONSTER_CONCLUSION'
  | 'WHITE_FALSE_VOICE_WARNING'
  | 'RADIO_SURVIVOR_CHORUS'
  | 'FATHER_RELAY_TRACE';
...
export type Guest = {
  ...
  offeredItems: {
    id: string;
    type: 'FOOD' | 'FUEL' | 'MEDICINE' | 'VALUABLE' | 'INFORMATION';
    name: string;
    short: string;
    detail: string;
  ...
  eventChain: {
    id: string;
    stage: 'ARRIVAL' | 'LIFE_AT_HOTEL' | 'CONFLICT' | 'RESOLUTION';
    title: string;
    completed: boolean;
  }[];
  infectionState:
...
export type StoryVisitorArrivalId =
  | 'samuel_rescue_survivor'
  | 'family_route_survivor';
...
export type NightPreparationCategory =
  | 'PATROL'
  | 'ISOLATION'
  | 'EXTERIOR_LIGHT'
  | 'NOISE';
export type NightPreparationOptionId =
  | 'STANDARD_WATCH'
  | 'ROVING_PATROL'
  | 'OPEN_FLOORS'
  | 'SEALED_WINGS'
  ...
  | 'NORMAL_HOURS'
  | 'SILENCE_PROTOCOL';
export type NightPreparationConfig = Record<
  NightPreparationCategory,
  NightPreparationOptionId
>;
export type NightPreparationEffect = {
  fuelCost?: number;
  hotelStats?: Partial<
    Pick<HotelStats, 'security' | 'crime' | 'hotelCondition'>
  >;
  threat?: number;
  allGuestStress?: number;
  diseaseChance?: number;
};
export type NightPreparationOptionDefinition = {
  id: NightPreparationOptionId;
  category: NightPreparationCategory;
  name: string;
  description: string;
  tradeoff: string;
  requiresPowerCircuit?: PowerCircuitId;
  effect: NightPreparationEffect;
};
...
export type HotelLogEntry = {
  day: number;
  type: 'CHECK_IN' | 'CHECK_OUT' | 'RESOURCE' | 'EVENT';
  message: string;
...
export type StaffDutyResult = {
  dutyId: Exclude<StaffDutyId, 'SCAVENGE'>;
  guestId: string;
  guestName: string;
  effect: string;
};
...
export type HotelStats = {
  ...
  waterSustainability: number;
  crime: number;
  survivorPopulation: number;
  averageTrust: number;
  resources: number;
};
...
export type VisitorReactionDefinition = {
  ...
  guestId: string;
  label: string;
  faction: keyof Reputations;
  dialogue: string;
  requiredFlags?: EventFlags;
  forbiddenFlags?: string[];
  minimumReputation?: Partial<Reputations>;
  maximumReputation?: Partial<Reputations>;
  trustDelta: number;
  riskDelta?: number;
  offerBonus?: Partial<Resources>;
};
...
export type FacilityLevelDefinition = {
  ...
  statChanges: Partial<HotelStats>;
  reputationChanges: Partial<Reputations>;
};
...
export type NightEventCondition = {
  ...
  maximumSecurity?: number;
  maximumResource?: Partial<Resources>;
  shortage?: 'food' | 'water';
  requiresGuests?: boolean;
  requiredEmptyRoomNumber?: number;
  requiredFlags?: EventFlags;
  requiredAnyFlags?: EventFlags;
  forbiddenFlags?: string[];
  relationship?: {
    sourceId: string;
    targetId: string;
    minimumWeightedValue?: number;
    maximumWeightedValue?: number;
  };
};
export type NightEventEffect = {
  resources?: Partial<Resources>;
  hotelStats?: Partial<HotelStats>;
  reputations?: Partial<Reputations>;
  flags?: EventFlags;
  threat?: number;
  fatherStoryProgress?: number;
  allGuestStress?: number;
  targetGuestHealth?: number;
  targetGuestHealthMinimum?: number;
  roomChange?: {
    roomNumber: number;
    status: 'DAMAGED' | 'LOCKED';
    roomCondition: number;
  };
  openCaseId?: InvestigationCaseId;
  monsterKnowledgeSourceIds?: MonsterKnowledgeSourceId[];
  guestEffects?: {
    guestId: string;
    trust?: number;
    stress?: number;
...
export type NightEventChoice = {
  id: string;
  label: string;
  description: string;
  requiredResources?: Partial<Resources>;
  effect: NightEventEffect;
};
export type NightEventDefinition = {
  id: string;
  title: string;
  description: string;
  quote: string;
  priority: number;
  condition: NightEventCondition;
  choices: NightEventChoice[];
};
export type StoryChoiceEffect = {
  ...
  threat?: number;
  fatherStoryProgress?: number;
  relationship?: { targetId: string; delta: number };
  discoverTrait?: string;
  departure?: { endingState: string; finalState: string };
  scheduledVisitorArrival?: { id: StoryVisitorArrivalId; delayDays: number };
};
...
export type CutsceneId =
  ...
  | 'room_body_discovery'
  | 'father_radio_signal'
  | 'father_at_gate_quarantine'
  | 'father_at_gate_opened'
  | 'hotel_siege_held'
  | 'hotel_siege_retreat'
  | 'mia_daniel_reunion'
  | 'daniel_mia_choice'
  | 'carter_safe_passage'
  | 'walter_archive_opened'
  | 'walter_key_hidden'
  | 'eleanor_clinic_opened'
  | 'eleanor_mobile_rounds'
  | 'hazel_perimeter_watch'
  | 'hazel_vengeance_expedition'
  | 'thomas_microgrid_online'
  | 'thomas_radio_relay'
  | 'noah_community_table'
  | 'noah_ration_lab'
  | 'samuel_civil_guard'
  | 'samuel_rescue_patrol'
  | 'ruth_care_team'
  | 'ruth_field_nurse'
  | 'rosa_household_network'
  | 'rosa_family_zone'
  | 'eli_quartermaster'
  | 'eli_safe_passage'
  | 'claire_nursery'
  | 'claire_medical_passage'
  | 'grace_mutual_aid'
  | 'grace_pilgrimage'
  | 'vale_behavior_map'
  | 'vale_research_destroyed'
  | 'owen_defense_force'
  | 'owen_escape_route'
  | 'hayes_command_signed'
  | 'hayes_civilian_command'
  | 'jack_fair_exchange'
  | 'jack_monopoly_market'
  | 'victor_public_trust'
  | 'victor_monopoly_alliance'
  | 'lily_truth_broadcast'
  | 'lily_truth_archive'
  | 'white_door_accepted'
  | 'white_banished';
export type CutsceneDefinition = {
  ...
  triggerStoryEventId?: string;
  triggerStoryChoiceId?: string;
  priority: number;
  minimumCompletedDay?: number;
  maximumCompletedDay?: number;
  kicker: string;
  title: string;
  body: string;
  quote: string;
...
export type InvestigationPointDefinition = {
  id: InvestigationPointId;
  name: string;
  description: string;
  finding: string;
...
export type InvestigationConclusionDefinition = {
  id: InvestigationConclusionId;
  label: string;
  description: string;
  supportedBy: EvidenceId[];
  contradictedBy: EvidenceId[];
  minimumSupport: number;
  supportedOnlyFlags?: string[];
  effect: {
    hotelStats?: Partial<HotelStats>;
    reputations?: Partial<Reputations>;
    flags: EventFlags;
...
export type InvestigationCaseDefinition = {
  id: InvestigationCaseId;
  title: string;
  summary: string;
  relatedRoom: number;
  minimumEvidenceToConclude: number;
  points: InvestigationPointDefinition[];
  conclusions: InvestigationConclusionDefinition[];
  correctConclusionId: InvestigationConclusionId;
  correctFlag: string;
};
export type InvestigationCaseState = {
  ...
  openedDay: number;
  inspectedPointIds: InvestigationPointId[];
  collectedEvidenceIds: EvidenceId[];
  conclusionId: InvestigationConclusionId | null;
  resolvedDay: number | null;
};
export type VisitorStatementDefinition = {
  id: VisitorStatementId;
  guestId: string;
  questionId: string;
  claim: string;
  finding: string;
  assessment: VisitorStatementAssessment;
  requiredInspectedItemId?: string;
  knowledgeSourceId: MonsterKnowledgeSourceId;
};
export type VisitorStatementRecord = {
  statementId: VisitorStatementId;
  guestId: string;
  questionId: string;
  recordedDay: number;
  assessment: VisitorStatementAssessment;
};
export type MonsterInsightDefinition = {
  id: MonsterInsightId;
  name: string;
  description: string;
};
export type MonsterCodexEntryDefinition = {
  id: MonsterCodexEntryId;
  name: string;
  ...
  preparationCountermeasure?: {
    ...
    effect: Omit<NightPreparationEffect, 'fuelCost'>;
  };
  insights: MonsterInsightDefinition[];
};
export type MonsterKnowledgeSourceDefinition = {
  id: MonsterKnowledgeSourceId;
  entryId: MonsterCodexEntryId;
  insightId: MonsterInsightId;
  name: string;
  certainty: MonsterKnowledgeCertainty;
};
export type MonsterCodexEntryState = {
  entryId: MonsterCodexEntryId;
  sourceIds: MonsterKnowledgeSourceId[];
  insightIds: MonsterInsightId[];
  updatedDay: number;
};
