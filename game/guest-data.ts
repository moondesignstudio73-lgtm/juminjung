...
type Seed = {
  ...
  offer?:Partial<Resources>; items?:Guest["offeredItems"]; risk?:number; condition?:string; intro?:string; arrivalConditions?:Guest["arrivalConditions"];
};
...
const seeds:Seed[] = [
  {id:"eleanor",name:"엘리너 리드",age:31,gender:"여성",role:"응급의학과 의사",description:"침착하고 효율을 우선하는 의사.",days:[1,4],stay:2,traits:["Calm","Doctor"],hidden:["TriageGuilt"],aura:aura("medical-care-zone","Medical Care Zone","diseaseChance",0,"주변 NORMAL_DISEASE 확률을 0%로 낮춘다.","NORMAL_DISEASE"),relations:[{targetId:"ruth",type:"ALLY",value:40},{targetId:"claire",type:"PATIENT",value:30},{targetId:"victor",type:"ETHICAL_CONFLICT",value:-35}],offer:{food:3,fuel:8,medicine:3},items:[item("medicine","MEDICINE","의약품 ×3","밀봉된 응급 의약품."),item("bandage","MEDICINE","붕대 ×2","깨끗한 의료용 붕대."),item("medical-id","INFORMATION","의료인 신분증","세인트 머시 병원 신분증.")]},
  {id:"walter",name:"월터 브릭스",age:67,gender:"남성",role:"전직 정비공",description:"퉁명스럽지만 책임감이 강한 아버지의 옛 친구.",days:[2,6],stay:4,traits:["Responsible","Mechanic"],hidden:["FatherOldFriend"],aura:aura("maintenance-zone","Maintenance Zone","breakdownRisk",-25,"주변 객실과 시설의 고장 위험 감소."),relations:[{targetId:"father",type:"OLD_FRIEND",value:70}],offer:{parts:4,fuel:2},items:[item("parts","VALUABLE","부품 ×4","발전기에 맞는 정비 부품."),item("toolbox","VALUABLE","낡은 공구함","호텔 열쇠와 같은 문양이 있다.")]},
  {id:"mia",name:"미아 카터",age:9,gender:"여성",role:"어린이",description:"토끼 인형을 놓지 않는 말없는 아이.",days:[3,7],stay:5,traits:["Child","Quiet"],hidden:["InconsistentStory"],aura:aura("comfort-presence","Comfort Presence","stress",-8,"인접한 보호 성향 NPC의 Stress 감소."),relations:[{targetId:"daniel",type:"UNKNOWN_FAMILY",value:0}],offer:{food:1},items:[item("rabbit","VALUABLE","토끼 인형","한쪽 귀에 마른 피가 묻어 있다."),item("candy","FOOD","사탕 ×1","아이에게 남은 유일한 식량.")]},
  {id:"daniel",name:"다니엘 카터",age:38,gender:"남성",role:"실종자를 찾는 생존자",description:"미아의 아버지라고 주장하는 절박한 남자.",days:[6,12],stay:3,traits:["Persistent"],hidden:["UnverifiedFamily"],aura:aura("family-bond","Family Bond","stress",-12,"Mia 근처에서만 신뢰와 안정 증가."),relations:[{targetId:"mia",type:"UNKNOWN_FAMILY",value:0}],offer:{fuel:3},items:[item("photo","INFORMATION","가족사진","미아와 닮은 아이가 있지만 얼굴이 긁혀 있다.")]},
  {id:"samuel",name:"새뮤얼 프라이스",age:44,gender:"남성",role:"전직 경찰",description:"규칙과 질서를 중시하는 전직 경찰.",days:[4,9],stay:4,traits:["Authority","FormerPolice"],hidden:["AbandonedRefugees"],aura:aura("security-presence","Security Presence","security",12,"주변 도난과 폭력 위험 감소."),relations:[{targetId:"owen",type:"SUSPICION",value:-25}],offer:{security:5},items:[item("ammo","VALUABLE","탄약 ×6","사용 가능한 권총 탄약."),item("badge","INFORMATION","경찰 배지","재난 첫날의 긁힌 흔적이 있다.")]},
  {id:"ruth",name:"루스 벨",age:36,gender:"여성",role:"간호사",description:"상냥하고 희생적인 간호사.",days:[5,10],stay:5,traits:["Kind","Nurse"],hidden:["MonsterScratch"],aura:aura("nursing-care","Nursing Care","diseaseChance",8,"주변 Injury 회복 보조.","INJURY"),relations:[{targetId:"eleanor",type:"ALLY",value:40}],offer:{medicine:2},items:[item("bandage","MEDICINE","붕대 꾸러미","일부는 이미 사용됐다.")]},
  {id:"jack",name:"잭 먼로",age:29,gender:"남성",role:"떠돌이 상인",description:"능글맞고 여러 세력과 거래하는 상인.",days:[6,13],stay:2,traits:["Trader","Talkative"],hidden:["DoubleDealer"],aura:aura("trade-network","Trade Network","trade",15,"호텔 거래 효율 증가."),relations:[{targetId:"victor",type:"BUSINESS",value:25}],offer:{food:4,parts:2},items:[item("crate","VALUABLE","봉인된 거래 상자","내용물 목록과 실제 무게가 다르다.")]},
  {id:"grace",name:"그레이스 할로웨이",age:52,gender:"여성",role:"종교인",description:"차분하게 사람을 위로하는 신앙인.",days:[7,14],stay:6,traits:["Comforting","Religious"],hidden:["CultLeader"],aura:aura("faith","Faith","stress",-10,"주변 Stress 감소, 과학자와 갈등 가능."),relations:[{targetId:"vale",type:"IDEOLOGICAL_CONFLICT",value:-40}],offer:{food:2},items:[item("scripture","INFORMATION","낡은 경전","괴물 출현 날짜가 예언처럼 덧쓰여 있다.")]},
  {id:"owen",name:"오웬 밀러",age:24,gender:"남성",role:"탈영병",description:"민간인 공격 명령을 거부한 예민한 탈영병.",days:[8,14],stay:5,traits:["Just","FormerSoldier"],hidden:["Deserter"],aura:aura("combat-readiness","Combat Readiness","security",15,"주변 객실 습격 피해 감소."),relations:[{targetId:"hayes",type:"ENEMY",value:-80},{targetId:"samuel",type:"SUSPICION",value:-25}],offer:{security:6},items:[item("rifle","VALUABLE","손상된 소총","격발 장치가 고장 났다.")]},
  {id:"hayes",name:"마커스 헤이스 대령",age:55,gender:"남성",role:"군 잔존세력 지휘관",description:"호텔 편입을 노리는 강압적인 지휘관.",days:[11,17],stay:4,traits:["Military","Authoritarian"],hidden:["HotelTakeover"],aura:aura("military-control","Military Control","security",20,"같은 층 Security 증가, 자유도 감소."),relations:[{targetId:"owen",type:"ENEMY",value:-80}],offer:{food:8,fuel:6,security:8},items:[item("orders","INFORMATION","군 편입 명령서","호텔을 임시 군사시설로 지정한다.")]},
  {id:"lily",name:"릴리 포스터",age:27,gender:"여성",role:"기자",description:"괴물 최초 출현을 추적하는 집요한 기자.",days:[9,16],stay:5,traits:["Curious","Journalist"],hidden:["OriginDocuments"],aura:aura("information-network","Information Network","information",15,"숨겨진 이벤트 사전 경고 확률 증가."),relations:[{targetId:"vale",type:"INVESTIGATION",value:20}],offer:{parts:2},items:[item("files","INFORMATION","취재 파일","괴물 최초 출현 지역 문서.")]},
  ...
  {id:"victor",name:"빅터 케인",age:41,gender:"남성",role:"전 기업 임원",description:"사람을 자원 가치로 판단하는 정중한 협상가.",days:[11,18],stay:4,traits:["Executive","Calculating"],hidden:["BunkerMonopoly"],aura:aura("resource-optimization","Resource Optimization","trade",20,"거래 수익과 일부 자원 효율 증가."),relations:[{targetId:"rosa",type:"CLASS_CONFLICT",value:-30},{targetId:"eleanor",type:"ETHICAL_CONFLICT",value:-35}],offer:{food:10,fuel:10},items:[item("valuables","VALUABLE","귀금속 ×5","이제는 쓸모가 불분명한 귀금속.")]},
  {id:"rosa",name:"로사 마르티네즈",age:40,gender:"여성",role:"두 아이의 어머니",description:"공동체를 우선하는 기초 간병 경험자.",days:[12,19],stay:7,traits:["Parent","Community"],hidden:["CareExperience"],aura:aura("community-care","Community Care","stress",-8,"아이와 노약자의 Stress 감소."),relations:[{targetId:"victor",type:"CLASS_CONFLICT",value:-30}],offer:{food:3,water:4},items:[item("family-bag","FOOD","가족 비상식량","아이들과 나눌 마지막 식량.")]},
  ...
  {id:"vale",name:"에이드리언 베일 박사",age:48,gender:"남성",role:"생물학자",description:"괴물 출현 전 유사 생물을 연구한 생물학자.",days:[14,21],stay:6,traits:["Scientist","Calm"],hidden:["PreOutbreakResearch"],aura:aura("monster-analysis","Monster Analysis","information",20,"Monster Threat 이벤트 일부 예측."),relations:[{targetId:"lily",type:"INVESTIGATION",value:20},{targetId:"white",type:"UNKNOWN",value:0}],offer:{medicine:2},items:[item("sample","INFORMATION","조직 샘플","인간과 다른 조직이 든 밀봉관.")]},
  {id:"hazel",name:"헤이즐 퀸",age:34,gender:"여성",role:"사냥꾼",description:"괴물의 행동 패턴을 관찰하는 과묵한 사냥꾼.",days:[15,22],stay:5,traits:["Hunter","Observant"],hidden:["FamilyLost"],aura:aura("perimeter-watch","Perimeter Watch","monsterThreat",-12,"외곽 객실 배치 시 Monster Threat 감소."),relations:[{targetId:"vale",type:"SUSPICION",value:-20},{targetId:"white",type:"SUSPICION",value:-60}],offer:{food:4,security:4},items:[item("traps","VALUABLE","사냥 덫 ×2","괴물 발자국과 맞지 않는 털이 끼어 있다.")]},
  ...
  {id:"claire",name:"클레어 노박",age:26,gender:"여성",role:"임신한 생존자",description:"위험 세력과 연결된 아이의 아버지를 숨긴다.",days:[17,24],stay:8,traits:["Cautious","Pregnant"],hidden:["DangerousFather"],aura:aura("protective-instinct","Protective Instinct","trust",8,"보호 성향 NPC의 Trust 상승."),relations:[{targetId:"eleanor",type:"PATIENT",value:30}],offer:{medicine:1},items:[item("ultrasound","INFORMATION","초음파 사진","뒷면에 군부대 좌표가 적혀 있다.")]},
  {id:"white",name:"미스터 화이트",age:0,gender:"불명",role:"불명",description:"먼지 하나 없는 흰 셔츠를 입은 정중한 남자.",days:[18,26],stay:3,traits:["Polite","Unknown"],hidden:["MonsterRelated","NonHumanPossible"],aura:aura("unknown-presence","Unknown Presence","stress",12,"주변 사건과 Stress를 비정상적으로 변화시킨다."),relations:[{targetId:"vale",type:"UNKNOWN",value:0},{targetId:"hazel",type:"FEAR",value:-50}],offer:{},items:[],risk:85,condition:"상처 없음 · 비정상"},
];
...
export function createGuests():Guest[] {
  return seeds.map((s) => ({
    id:s.id,name:s.name,age:s.age,gender:s.gender,role:s.role,description:s.description,portrait:`/assets/portraits/${s.id}/neutral.png`,expressions:["neutral","happy","sad","angry","afraid","suspicious","injured"],
